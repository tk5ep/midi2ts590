...
########################################
    try:
        ...
        if Config.has_section('Radio'):
            ...
            config.rxtimeout=Config.getint('Radio','rxtimeout')
            config.txtimeout=Config.getint('Radio','txtimeout')
...
########################################
    ...
    Config.set('Default','tuningstep','1')
    Config.set('Default','radiopoll','1')
    # add section Midi
...
def DJ_scan():
        ...
        if device == 0xB0:                              # a pot/slider/jog has been turned
            if status == 48:                            # JOG A activity detected
                ...
                if DEBUG:
                    print (math.floor(( control / 11)))                 # SL command 00-11
                strCat = format ("SL%02d"% (math.floor( control / 11 )))
                ts590.query(strCat,0)
            elif status == 60 and config.RadioMode =='CW':              # CW bandwidth 050-2500
                if DEBUG:
                    print ("%04d"% (math.floor(50 + (2500 - 50) * control / 127 )))
                strCat = format ("FW%04d"% (math.floor(50 + (2500 - 50) * control / 127 )))
                ts590.query(strCat,0)
                ...
                if DEBUG:
                    print (math.floor(( control / 9.5 )))               # SH command 00-13
    ...
    if answerIF != None:                            # if we have a valid answer
        ...
        if split == '0':                            # split off
            if VFO == '0':                          # if VFO A main
                DJ_LedDA_SYNC(1)                    # LED VFO A on
                DJ_LedDA_CUE(0)                     # LED VFO B off
                DJ_LedDA_KP3(0)                     # LED split A/B off
                DJ_LedDA_KP4(0)                     # LED split B/A off
            elif VFO == '1':
                DJ_LedDA_SYNC(0)                    # LED VFO B off
                DJ_LedDA_CUE(1)                     # LED VFO A on
                DJ_LedDA_KP3(0)                     # LED split A/B off
                DJ_LedDA_KP4(0)                     # LED split B/A off
        if split == '1':            # split on
...
if __name__ == '__main__':
    ...
    print("\nFor a 'clean' stop of this software, use CTRL-C twice.")

    if config.RadioPoll:
        daemon = Thread(target=pollRadio, args=(
    ...
    while True:
        try:
            ...
            if config.RadioPoll:
                ...
                daemon.join()                                           # join the thread to stop it
                input('CTRL-C to QUIT')
                print('Poll thread killed')
